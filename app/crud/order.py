...
def create_order(order_data: OrderCreate, user_id: str):
    ...
    if len(cart["items"]) == 0:
       return {"message":"Add Items to the Cart to Order"}
    else:
        result=0.0
        total=0.0
...
def order_detailed(order_id:str):
    odr=order_collection.find_one({"_id":ObjectId(order_id)})
    return Order(**{"order_id":str(odr["_id"]),**odr})